...
成功编译后，PDF 预览会在可用时使用 SyncTeX 元数据。双击 PDF 中渲染出的某个单词时，编辑器会结合 PDF 单词框和多点 SyncTeX 采样跳转到匹配的 LaTeX 源文件，并选中对应的源码 token，避免退化成大范围行级选中。当你在已经打开的 PDF 预览中点击编译时，刷新后的 PDF 会回到点击编译时的同一页内滚动位置，而不是跳回首页。没有 SyncTeX 数据的旧构建需要重新编译后才能使用 PDF 到源码跳转。
